...
%classdef VAPP_AST_Node < handle & matlab.mixin.Copyable
    ...
    methods
        ...
        function self = VAPP_AST_Node(uniqID_generator)
            ...
            self.alias='';
            self.note='';
            self.note.contrib_type_change=''; 
            self.note.multiple_power_func_used=0; % counts the number of pow(a,xxx) used 
        end

        function out = accept_visitor(self, visitor)
            % here comes a hack with the eval method in order to call the right
            % method of the visitor. explain this here.
        ...
        function out = get_alias(self)
            out = self.alias;
        end
        
        function out = get_note(self, note_field)
            out = self.note.(note_field);
        end
        ...
        function set_type(self, Type)
            ...
            if isoctave()
                ...
                self.visitMethod = eval(['@visit', upper(Type(1)), Type(2:end)]);
            end
        end
        ...
        function set_alias(self, alias)
            self.alias = alias;
        end
        
        function set_note(self, note_field, note)
            self.note.(note_field) = note;
        end
        
        % methods for handling attrs
        
        function out = get_attr(self, attr)
            if isfield(self.attrs, attr)
                out = self.attrs.(attr);
            else
                out = '';
            end
        ...
        function add_range(self, tok_or_AST)
            ...
            elseif isstruct(tok_or_AST)
                ...
                file_total = length(self.pos);
                
                if file_total == 0
                    % no existing entry with same filename found
                    % create a new one
                    
                        self.pos{1}.infile_path = tok.infile_path;
                        
                        if isfield(tok,'macro_path')
                            self.pos{1}.macro_path = tok.macro_path;
                        else
                            self.pos{1}.macro_path = '';
                        end
                        
                        self.pos{1}.lineno = tok.lineno;
                        self.pos{1}.linepos = [0 0];
                        self.pos{1}.range = [0 0];
                        
                else
                    % has files record
                    for index = 1:(file_total+1)
                        % for each file
                        
                        if index == (file_total+1)
                            % no existing entry with same filename found
                            % create a new one
                            self.pos{index}.infile_path = tok.infile_path;
                            
                            if isfield(tok,'macro_path')
                                self.pos{index}.macro_path = tok.macro_path;
                            else
                                self.pos{index}.macro_path = '';
                            end
                            
                            self.pos{index}.lineno = [];
                            self.pos{index}.lineno = tok.lineno;
                            self.pos{index}.linepos = [0 0];
                            self.pos{index}.range = [0 0];
                        ...
                        if strcmp(self.pos{index}.infile_path, tok.infile_path)
                            ...
                            self.pos{index}.lineno(2) = tok.lineno(2);
                            self.pos{index}.linepos = [0 0];
        ...
        function add_valint_result(self, valint_rule, valint_msg_type, valint_msg, rule_id, opt)
            % valint_rule: the specific rule
            % valint_msg: customized message for this rule
            self.valint_result{end+1} = {valint_rule, valint_msg_type, valint_msg, rule_id, opt};
        end
