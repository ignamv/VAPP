function [tok_list, new_macro_definitions] = ...
    ...
    % End of DEBUG
    if ~strcmp(str_utils.output_format, 'default')
        fprintf(['[status] [' infile_path '] [line 20] [line 0]:  Tokenizing ......\n']);
    end
    ...
    line_count = 1;
    for index = 1:length(s_newline_mask)
        if (s_newline_mask(index) == 1)
            line_count = line_count+1;
        end
        
        s_newline_mask(index) = line_count;
    end
    ...
    for index = 1:length(tok_list)
        ...
        % line number
        this_lineno_start = s_newline_mask(this_tok.range(1));
        this_lineno_end = s_newline_mask(this_tok.range(2));
