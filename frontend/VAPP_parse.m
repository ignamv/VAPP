function AST = VAPP_parse(toks, str_utils)
    ...
    while next_pos <= length(toks)
        if ~strcmp(str_utils.output_format, 'default')
            fprintf('[status] [ ] [line %d] [line 0] [ID 0]:  Parsing ......\n', fix(next_pos/length(toks)*100));
        end
...
function test()
    ...
    [toks, oof_1, oof_2] = VAPP_tokenize(case_stmt, {}, {}, {}, struct(), ...
                                         str_utils);
    [AST, new_next_pos] = ...
        parse_case_statement(toks, 1, contrib_allowed, uniqID_generator, ...
                             str_utils);
...
function out = convert_const_dtype(from_type, to_type, key)
    ...
    if ~success
        ...
        error(['Not a valid constant ', err_type]);
    end

end

function assert_keyword(tok, kw)
    if nargin == 1
        if ~is_keyword(tok)
            error('Expected a keyword token, but did not find it');
        end
    else
        if ~is_keyword(tok, kw)
            error(sprintf('Expected keyword "%s", but did not find it', kw));
        end
    end
end

function assert_one_of_the_keywords(tok, kw_arr, str_utils)
    if ~is_one_of_the_keywords(tok, kw_arr, str_utils)
        s = str_utils.str_arr_to_str(kw_arr);
        error(['Expected one of the keywords ', s, ', but did not find it']);
    end
end

function assert_symbol(tok, sym)
    if nargin == 1
        if ~is_symbol(tok)
            error('Expected a punctuation symbol token, but did not find it');
        end
    else
        if ~is_symbol(tok, sym)
            error(sprintf('Expected symbol "%s", but did not find it', sym));
        end
    end
end

function assert_one_of_the_symbols(tok, sym_arr, str_utils)
    if ~is_one_of_the_symbols(tok, sym_arr, str_utils)
        s = str_utils.str_arr_to_str(sym_arr);
        error(['Expected one of the symbols ', s, ', but did not find it']);
    end
end

function assert_one_of_the_symbols_or_keywords(tok, ...
                                               sym_arr, ...
                                               kw_arr, ...
                                               str_utils)
    ...
    if ~is_one_of_the_symbols_or_keywords(tok, sym_arr, kw_arr, str_utils)
        ...
        error(['Expected one of the symbols ', s_sym, ...
               ' or one of the keywords ', s_kw, ', but did not find it']);
...
function assert_identifier(tok, id_name)
    if nargin == 1
        if ~is_identifier(tok)
            error('Expected an identifier token, but did not find it');
        end
    else
        if ~is_identifier(tok, id_name)
            error(sprintf('Expected identifier "%s", but did not find it', ...
                          id_name));
        end
    end
end

function assert_one_of_the_identifiers(tok, id_arr, str_utils)
    if ~is_one_of_the_identifiers(tok, id_arr, str_utils)
        s = str_utils.str_arr_to_str(id_arr);
        error(['Expected one of the identifiers ', s, ', but did not find it']);
    end
end

function assert_simulator_directive(tok)
    if ~is_simulator_directive(tok)
        error('Expected a simulator directive token, but did not find it');
    end
end

function assert_string(tok)
    if ~is_string(tok)
        error('Expected a string token, but did not find it');
    end
end
...
function new_next_pos = skip_to_keyword(kw, toks, next_pos)
    ...
    error(['Keyword token "', kw, '" not found']);
...
function new_next_pos = skip_to_symbol(sym, toks, next_pos)
    ...
    error(['Symbol token "', sym, '" not found']);
...
function new_next_pos = ...
    ...
    s = str_utils.str_arr_to_str(sym_arr);
    error(['None of the symbols ', s, ' found']);
...
function new_next_pos = skip_to_one_of_the_symbols_or_keywords(sym_arr, ...
    ...
    s_sym = str_utils.str_arr_to_str(sym_arr);
    s_kw = str_utils.str_arr_to_str(kw_arr);
    error(['None of the symbols ', s_sym, ' or keywords ', s_kw, ' found']);

end

function [AST, new_next_pos] = ...
            parse_module(toks, next_pos, uniqID_generator, str_utils)
    ...
    while true
        if ~strcmp(str_utils.output_format, 'default')
            fprintf('[status] [Parsing] [line %d] [line 0] [ID 0]:  Module "%s"......\n', fix(next_pos/length(toks)*100), module_name);
        end
        ...
        elseif is_keyword(tok, 'electrical')
            [stmt, next_pos] = parse_electrical_statement(toks, next_pos, ...
                                                          uniqID_generator);
            addl_stmts = {stmt};
        ...
        elseif is_identifier(tok, 'thermal')
            [stmt, next_pos] = parse_thermal_statement(toks, next_pos, ...
                                                       uniqID_generator);
            addl_stmts = {stmt};
...
function [module_name, terminal_names, new_next_pos] = ...
    ...
    % the 'module' keyword
    assert_keyword(toks{next_pos}, 'module')
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
            parse_analog_section(toks, next_pos, uniqID_generator, str_utils)
...
function [AST, new_next_pos] = ...
            parse_analog_function(toks, next_pos, uniqID_generator, str_utils)
    ...
    tok = toks{next_pos};
    assert_keyword(tok, 'endfunction');
    next_pos = next_pos + 1;
...
function [AST_dtype, func_name, new_next_pos] = ...
    ...
    % the 'analog' keyword
    assert_keyword(toks{next_pos}, 'analog')
    next_pos = next_pos + 1;

    % the 'function' keyword
    assert_keyword(toks{next_pos}, 'function')
    next_pos = next_pos + 1;
...
function new_next_pos = parse_inout_statement(toks, next_pos)
    ...
    % the 'inout' keyword
    assert_keyword(toks{next_pos}, 'inout')
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    % the 'electrical' keyword
    assert_keyword(toks{next_pos}, 'electrical');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    % the 'aliasparam' keyword
    assert_keyword(toks{next_pos}, 'aliasparam');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    % the 'branch' keyword
    assert_keyword(toks{next_pos}, 'branch');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
            parse_thermal_statement(toks, next_pos, uniqID_generator)

    % A "thermal" statement begins at next_pos. It is of the form:
    %    * the 'thermal' identifier,
    %    * followed by a comma separated list of terminals (identifiers),
    %    * followed by a semicolon
    %
    % Examples: 
    ...
    %    thermal t1, t2;
    %
    % We build and return an AST node for this statement, of Type 'thermal', 
    % with no children and with 1 attribute 'terminals' (a cell array of 
    % strings holding the names of the terminals declared above)
    org_pos = next_pos;
    
    % the 'thermal' identifier
    assert_identifier(toks{next_pos}, 'thermal');
    next_pos = next_pos + 1;
    ...
    % build the AST and return
    AST = create_thermal_AST(terminal_names, uniqID_generator);
    AST_add_range(AST, toks, org_pos, next_pos-1);
...
function [AST, new_next_pos] = ...
    ...
    % the 'input' keyword
    assert_keyword(toks{next_pos}, 'input');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    % the 'output' keyword
    assert_keyword(toks{next_pos}, 'output');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    % the data type
    tok = toks{next_pos};
    assert_one_of_the_keywords(tok, {'integer', 'real', 'string'}, str_utils);
    VA_dtype = tok.value;
    AST_dtype = convert_const_dtype('VA_dtype', 'AST_dtype', VA_dtype);
    next_pos = next_pos + 1;
    ...
    while true
        ...
        tok = toks{next_pos};
        assert_one_of_the_symbols(tok, {',', ';'}, str_utils);
...
function [AST, new_next_pos] = parse_variable_declaration(AST_dtype, ...
    ...
    if is_initialized
        ...
        [var_name, expr_AST, comma_or_semicolon_pos] = ...
            parse_assignment(toks, next_pos, sym_arr, kw_arr, ...
                             uniqID_generator, str_utils);
    ...
    assert_one_of_the_symbols(toks{comma_or_semicolon_pos}, sym_arr, str_utils);
...
function [AST, new_next_pos] = ...
    ...
    [parm_name, parm_default_expr_AST, next_pos] = ...
        parse_assignment(toks, next_pos, sym_arr, kw_arr, uniqID_generator, ...
                         str_utils);
    tok = toks{next_pos};
    assert_one_of_the_symbols_or_keywords(tok, sym_arr, kw_arr, str_utils);
    ...
    while true
        ...
        [parm_range_AST, next_pos] = ...
            parse_parameter_range(toks, next_pos, uniqID_generator, str_utils);
        
        parm_range_children = [parm_range_children, {parm_range_AST}];

    end

    assert_one_of_the_symbols(tok, sym_arr, str_utils);
...
function [AST, new_next_pos] = ...
    ...
    assert_one_of_the_keywords(toks{next_pos}, kw_arr, str_utils);
    ...
    if str_utils.str_in_arr(Type, {'from', 'exclude_interval'})
        ...
        [child, next_pos] = ...
            parse_parameter_interval(toks, next_pos, uniqID_generator, ...
                                     str_utils);
...
function Type = detect_parm_range_Type(toks, next_pos, str_utils)
    ...
    tok = toks{next_pos};
    assert_one_of_the_keywords(tok, kw_arr, str_utils);
...
function [AST, new_next_pos] = ...
    ...
    elseif is_symbol(tok, '''')
        ...
        error('Expected one of {"(", "[", "''"}, but did not find it');

    end

    [AST, new_next_pos] = feval(parse_func, toks, next_pos, ...
                                uniqID_generator, str_utils);
    AST_add_range(AST, toks, org_pos, next_pos);
...
function [AST, new_next_pos] = ...
    ...
    elseif is_symbol(tok, '[')
        ...
        error('Expected an "(" or "[", but did not find it');
    end
    next_pos = next_pos + 1;
    ...
    sym_arr = {':'};
    [left_AST, next_pos] = ...
        parse_infty_or_expression(toks, next_pos, sym_arr, uniqID_generator, ...
                                  str_utils);
...
function [AST, new_next_pos] = parse_core_statement(toks, ...
    ...
    if is_identifier(tok)
        ...
        if is_symbol(tok2, '=')
            ...
            if ~contrib_allowed
                error('Contribution statements not allowed here')
            end
            [AST, new_next_pos] = ...
    ...
    elseif is_keyword(tok, 'for')
        % for loop
        [AST, new_next_pos] = ...
            parse_for_loop(toks, next_pos, contrib_allowed, ...
                           uniqID_generator, str_utils);
    ...
    elseif is_simulator_directive(tok)
        ...
        error('Expected a "core" statement, but did not find it');
...
function [AST, new_next_pos] = ...
    ...
    [var_name, expr_AST, semicolon_pos] = parse_assignment(toks, next_pos, ...
                                                           ...
                                                           str_utils);
    % the semicolon
    assert_symbol(toks{semicolon_pos}, ';');
...
function [AST, new_next_pos] = parse_if_statement(toks, ...
    ...
    [conditional_expr, close_paren_pos] = ...
        parse_and_skip_over_expression(toks, next_pos, sym_arr, kw_arr, ...
                                       uniqID_generator, str_utils);
    assert_symbol(toks{close_paren_pos}, ')');
...
function [AST, new_next_pos] = parse_case_statement(toks, ...
    ...
    if isempty(case_items_ASTs)
        error('Case statement has to contain at least one case item');
    end
...
function [AST, new_next_pos] = parse_case_item(toks, ...
    ...
    [case_candidates_AST, next_pos] = ...
        parse_case_candidates(toks, next_pos, uniqID_generator, str_utils);
    ...
    while true
        [AST, comma_or_colon_pos] = ...
            parse_and_skip_over_expression(toks, next_pos, sym_arr, kw_arr, ...
                                           uniqID_generator, str_utils);
        ASTs = [ASTs, {AST}];
        assert_one_of_the_symbols(toks{comma_or_colon_pos}, sym_arr, str_utils);
...
function [AST, new_next_pos] = parse_for_loop(toks, ...
    ...
    % the 'for' keyword
    assert_keyword(toks{next_pos}, 'for');
    next_pos = next_pos + 1;
    ...
    [assignment_1, next_pos] = ...
        parse_assignment_statement(toks, next_pos, uniqID_generator, str_utils);
...
function [AST, new_next_pos] = parse_repeat_loop(toks, ...
    ...
    % the 'repeat' keyword
    assert_keyword(toks{next_pos}, 'repeat');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = parse_while_loop(toks, ...
    ...
    % the 'while' keyword
    assert_keyword(toks{next_pos}, 'while');
    next_pos = next_pos + 1;
...
function [AST, new_next_pos] = ...
    ...
    assert_one_of_the_identifiers(toks{next_pos}, {'V', 'I', 'Pwr', 'Temp'}, str_utils);
    ...
    [lhs_AST, contrib_op_pos] = ...
        parse_and_skip_over_expression(toks, next_pos, sym_arr, kw_arr, ...
                                       uniqID_generator, str_utils);
    assert_symbol(toks{contrib_op_pos}, '<+');
...
function AST = parse_expression(toks, uniqID_generator, str_utils)
    ...
    while next_pos <= length(toks)
        ...
        if is_constant(tok)
            ...
            org_pos = next_pos;
            AST_add_range(const_AST, toks, org_pos, next_pos);
        ...
        elseif is_keyword(tok) || is_identifier(tok)
            ...
            [var_or_func_AST, next_pos] = ...
                parse_variable_or_function_call(toks, next_pos, ...
                                                uniqID_generator, str_utils);
        ...
        elseif is_bitwise_op(tok, str_utils) || is_shift_op(tok, str_utils)
            % bit-wise operators or shift operators
            if next_pos > 1
                prev_tok = toks{next_pos-1};
            else
                prev_tok = [];
            ...
            error('Unrecognized token while parsing expression');
    ...
    if length(expr_stack) ~= 1
        error('Expression stack does not contain exactly one item');
    end
...
function [AST, terminator_pos] = ...
    ...
    AST = parse_expression(expr_toks, uniqID_generator, str_utils);
    ...
    while true
        ...
        elseif is_symbol(tok, ':')
            if isempty(stack)
                ...
                [top, stack] = stack_pop(stack);
                if top ~= '?'
                    error('Mismatched conditional (?:) operator');
                end
            end
        ...
        elseif is_symbol(tok, ',')
            if isempty(stack)
                ...
                if top ~= '('
                    error('Improper use of comma in expression');
                end
            end
...
function [AST, new_next_pos] = ...
    ...
    if strcmp(var_or_func, 'var')
        ...
        [AST, new_next_pos] = parse_function_call(toks, next_pos, ...
                                                  uniqID_generator, str_utils);
...
function [AST, new_next_pos] = ...
            parse_function_call(toks, next_pos, uniqID_generator, str_utils)
    ...
    if ~is_keyword(tok) && ~is_identifier(tok)
        error('Expected a keyword/identifier, but did not find it');
    end
    func_name = tok.value;
    next_pos = next_pos + 1;

    % the arguments
    if str_utils.str_in_arr(func_name, {'V', 'I'})
        % V or I argument list
        [arg_nodes, next_pos] = ...
            parse_V_or_I_args(toks, next_pos, uniqID_generator);
...
function [ASTs, new_next_pos] = ...
    ...
    if isempty(ASTs)
        error('List of ports/identifiers cannot be empty');
    end
...
function [ASTs, new_next_pos] = ...
            parse_actual_args(toks, next_pos, uniqID_generator, str_utils)
    ...
    while true
        [AST, comma_or_close_paren_pos] = ...
            parse_and_skip_over_expression(toks, next_pos, sym_arr, kw_arr, ...
                                           uniqID_generator, str_utils);
        ASTs = [ASTs, {AST}];
        assert_one_of_the_symbols(toks{comma_or_close_paren_pos}, sym_arr, ...
                                  str_utils);
...
function out = is_bitwise_op(tok, str_utils)
...
function out = is_shift_op(tok, str_utils)
...
function [prec, assoc] = get_precedence_and_associativity(op, str_utils)
    ...
    error(sprintf('Operator %s not found in precedence table', op));
...
function [AST, new_next_pos] = parse_sim_variable(toks, next_pos, ...
    ...
    tok = toks{next_pos};
    assert_simulator_directive(tok);
...
function [AST, new_next_pos] = ...
            parse_sim_function_call(toks, next_pos, uniqID_generator, str_utils)
...
function AST = create_V_or_I_AST(V_or_I, children, uniqID_generator)
...
function AST = create_thermal_AST(terminal_names, uniqID_generator)

    AST = VAPP_AST_Node(uniqID_generator);
    AST.set_type('thermal');
    AST.set_attr('terminals', terminal_names);
