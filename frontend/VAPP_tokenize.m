function [tok_list, new_macro_definitions, new_context_stack] = ...
    ...
    tok_list{100000} = []; tok_index = 1;
    while next_pos <= length(input_str)
        
        if ~strcmp(str_utils.output_format, 'default')
            fprintf('[status] [%s] [line %d] [line 0] [ID 0]:  Tokenizing ......\n', parms.infile_path, fix(next_pos/length(input_str)*100));
        end
        ...
        elseif (str_utils.is_alpha(ch) || ch == '_' || ch == '@')
        ...
        elseif str_utils.ch_in_str(ch, punctuation_indicator_str)
            ...
            error(['Character "', ch, '" does not begin a valid token']);
        ...
        if ~isempty(toks) && is_permissive(context_stack)
            ...
            tok_list(tok_index:(length(toks)+tok_index-1)) = toks; tok_index = length(toks)+tok_index;
        end
    ...
    tok_list = tok_list(1:tok_index-1);
...
function [out, new_next_pos] = parse_identifier(input_str, next_pos, str_utils)
    ...
    if (~str_utils.is_alpha(ch) && ch ~= '_' && ch ~= '@')
...
function [new_macro_definitions, new_next_pos] = ...
    ...
    % insert the original file path information of this macro
    new_macro_definitions{end}.infile_path = parms.infile_path;
end
...
function [toks, new_next_pos] = ...
    ...
    % mark all toks as Macros
    for index = 1:length(toks)
        toks{index}.macro_path = macro.infile_path;
    end
end
